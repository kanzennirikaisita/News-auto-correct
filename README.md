...
ブラウザで `http://localhost:8000/` を開きます。Windowsでは環境に応じて `python` を `py` に置き換えます。`file://` ではfetch・Service Workerが動きません。HTTPSまたはlocalhostが必要です。
...
既存の記事が変われば同じIDのままrevisionを進め、既読状態を再判定します。初回に古い公開記事を大量投入せず、原則30日以内の公開・更新を採用します。最終変化検知から30日、最大3,000件を保持。各ソースは最大200件を処理、GitHub Releasesは直近30件です。取得上限を超える量の更新は取りこぼす可能性があります。

HTMLリンク方式はタイトル・リンクの変化だけを扱い、本文差分や正確な公開日時を扱いません。公開日時がない項目はnullとし、検知日時を表示します。
